...
export function Office3D() {
  // TODO fix the any later
  const { nodes, materials } : any = useGLTF('lib/landingPage/OfficeRoom.gltf')
  const ref = useRef();
  const tl = useRef();
  ...
  return (
    <group dispose={null}>
      <group rotation={[Math.PI / 1.9, 0, Math.PI / 3]} scale={0.01} position= {[0, -2.5, -3]}>
        ...
        <group position= {[0, -2.5, -3]} rotation={[Math.PI / 1.9, 0, Math.PI / 3]}>
